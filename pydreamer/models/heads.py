...
class DirectHead(nn.Module):
    ...
    def __init__(self, decoder):
        super().__init__()
        self.decoder: DenseDecoder = decoder

    def forward(self, state, obs, do_image_pred=False):
        obs_pred = self.decoder.forward(state)
        return (obs_pred, )

    def loss(self, obs_pred: TensorNBICHW, obs_target: TensorNBICHW, map_coord: TensorNBI4, map_seen_mask: Tensor):
        loss = self.decoder.loss(obs_pred, obs_target)  # (N,B,I)
        loss = -logavgexp(-loss, dim=-1)  # (N,B,I) => (N,B)
        with torch.no_grad():
            acc_map = self.decoder.accuracy(obs_pred, obs_target, map_coord)
            acc_map_seen = self.decoder.accuracy(obs_pred, obs_target, map_coord, map_seen_mask)
    ...
    def predict(self, obs_pred) -> Tensor:
        ...
        return self.decoder.to_distr(obs_pred)
...
class VAEHead(nn.Module):
    ...
    def __init__(self, encoder: ConvEncoder, decoder: ConvDecoder, state_dim=230, hidden_dim=200, latent_dim=30):
        ...
        self.prior_mlp = nn.Sequential(nn.Linear(state_dim, hidden_dim),
                                        nn.ELU(),
                                        nn.Linear(hidden_dim, 2 * latent_dim))

        self.post_mlp = nn.Sequential(nn.Linear(state_dim + encoder.out_dim, hidden_dim),
                                       nn.ELU(),
                                       nn.Linear(hidden_dim, 2 * latent_dim))

    def forward(self, state, obs, do_image_pred=False):
        embed = self.encoder.forward(obs)
        prior = self.prior_mlp(state)
        post = self.post_mlp(torch.cat([state, embed], -1))
        sample = diag_normal(post).rsample()
        obs_rec = self.decoder(torch.cat([state, sample], -1))
        if do_image_pred:
            prior_sample = diag_normal(prior).sample()
            obs_pred = self.decoder(torch.cat([state, prior_sample], -1))
        else:
            obs_pred = None
    ...
    def loss(self,
        ...
        loss_rec = self.decoder.loss(obs_rec, obs_target)
    ...
    def predict(self, obs_rec, prior, post, obs_pred) -> Tensor:
        ...
        if obs_pred is not None:
            ...
            return self.decoder.to_distr(obs_rec)
